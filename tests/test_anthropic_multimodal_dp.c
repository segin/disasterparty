...
int main(int argc, char *argv[]) {
    ...
    else { image_path = getenv("ANTHROPIC_TEST_IMAGE_PATH"); if (image_path) printf("Using image path from ANTHROPIC_TEST_IMAGE_PATH: %s\n", image_path);
           else { fprintf(stderr, "Usage: %s [path_to_image.jpg/png] or set ANTHROPIC_TEST_IMAGE_PATH\n", argv[0]);}}

    dp_context_t* context = dp_init_context(DP_PROVIDER_ANTHROPIC, api_key, NULL);
    if (!context) {
        fprintf(stderr, "Failed to initialize Disaster Party context for Anthropic (Multimodal).\n");
        curl_global_cleanup();
        return 1;
    }
    ...
    if (!dp_message_add_text_part(&messages[0], "Describe this image in detail.")) {
        fprintf(stderr, "Failed to add text part to Anthropic multimodal message.\n");
    ...
    if (image_path) {
        ...
        if (image_buffer) {
            ...
            if (base64_image_data) {
                ...
                if (!dp_message_add_base64_image_part(&messages[0], mime_type, base64_image_data)) {
                    fprintf(stderr, "Failed to add base64 image part to Anthropic message.\n");
        ...
        fprintf(stderr, "FAIL: Image path is required for multimodal test.\n");
    ...
    printf("Sending multimodal request to Anthropic model: %s\n", request_config.model);
    ...
    if (result == 0 && response.num_parts > 0 && response.parts[0].type == DP_CONTENT_PART_TEXT) {
        printf("\n--- Anthropic Multimodal Completion Response (HTTP %ld) ---\n", response.http_status_code);
        printf("%s\n", response.parts[0].text);
        if (response.finish_reason) printf("Finish Reason: %s\n", response.finish_reason);
        printf("---------------------------------------------------\n");
    } else {
        fprintf(stderr, "\n--- Anthropic Multimodal Completion Failed (HTTP %ld) ---\n", response.http_status_code);
