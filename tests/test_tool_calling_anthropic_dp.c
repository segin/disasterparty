...
int main() {
    ...
    const char* model_to_use = model_env ? model_env : "claude-3-opus-20240229";
    ...
    if (!dp_message_add_text_part(&messages[0], "What is the weather in London?")) {
    ...
    const char* mock_response = "{\"temperature\": \"12\", \"unit\": \"celsius\", \"description\": \"Rainy\"}";
    ...
    if (result == 0 && response2.http_status_code == 200) {
        for (size_t i = 0; i < response2.num_parts; ++i) {
            if (response2.parts[i].type == DP_CONTENT_PART_TEXT) {
                ...
                if (strstr(response2.parts[i].text, "12") || strstr(response2.parts[i].text, "Rainy") || strstr(response2.parts[i].text, "London")) {
                    step2_success = true;
                }
            }
        }
