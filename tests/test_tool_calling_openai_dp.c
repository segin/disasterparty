...
int main() {
    ...
    const char* model_to_use = model_env ? model_env : "gpt-4-turbo"; 
    ...
    if (!dp_message_add_text_part(&messages[0], "What is the weather in Tokyo?")) {
    ...
    // We must reconstruct the previous assistant message exactly as the API expects context.
    ...
    const char* mock_response = "{\"temperature\": \"22\", \"unit\": \"celsius\", \"description\": \"Sunny and clear\"}";
    ...
    if (result == 0 && response2.http_status_code == 200) {
        for (size_t i = 0; i < response2.num_parts; ++i) {
            if (response2.parts[i].type == DP_CONTENT_PART_TEXT) {
                ...
                if (strstr(response2.parts[i].text, "22") || strstr(response2.parts[i].text, "Sunny") || strstr(response2.parts[i].text, "Tokyo")) {
                    step2_success = true;
                }
            }
        }
