...
int main() {
    ...
    printf("Testing Gemini token counting functionality...\n");
    ...
    if (result == 0 && system_count > 0) {
        ...
        printf("~ System prompt token counting skipped (not supported by this provider)\n");
        // Don't count as failure since system prompt token counting might not be supported
    }
    ...
    for (size_t i = 0; i < sizeof(gemini_models) / sizeof(gemini_models[0]); i++) {
        request_config.model = gemini_models[i];
        size_t model_count = 0;
        result = dp_count_tokens(context, &request_config, &model_count);
        
        if (result == 0 && model_count > 0) {
            printf("✓ Model %s: %zu tokens\n", gemini_models[i], model_count);
        } else {
    ...
    // Test 7: Multimodal content (if supported)
    printf("\n=== Test 7: Multimodal content ===\n");
    
    dp_message_t multimodal_msg = {0};
    multimodal_msg.role = DP_ROLE_USER;
    dp_message_add_text_part(&multimodal_msg, "Describe this image:");
    // Add a small base64 image (1x1 pixel PNG)
    const char* tiny_png_base64 = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNkYPhfDwAChAGA60e6kgAAAABJRU5ErkJggg==";
    dp_message_add_base64_image_part(&multimodal_msg, "image/png", tiny_png_base64);
    ...
    if (result == 0 && multimodal_count > 0) {
        printf("✓ Successfully counted tokens for multimodal content: %zu\n", multimodal_count);
    } else {
        printf("~ Multimodal token counting skipped (not supported or failed)\n");
        // Don't count as failure since multimodal token counting might not be supported
    }
    
    dp_free_messages(&multimodal_msg, 1);
    ...
    printf("\n=== Test Results ===\n");
    if (test_failures == 0) {
        printf("✓ All Gemini token counting tests passed!\n");
        printf("Gemini token counting test (Disaster Party) finished successfully.\n");
        return EXIT_SUCCESS;
    } else {
        printf("✗ %d test(s) failed\n", test_failures);
        printf("Gemini token counting test (Disaster Party) finished with failures.\n");
        return EXIT_FAILURE;
    }
}